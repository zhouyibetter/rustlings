...
impl From<&str> for Person {
    fn from(s: &str) -> Self {
        ...
        // 2. Check if the split result has exactly 2 elements
        if parts.len() != 2 {
            return Person::default();
        }

        // 3. Use the first element as the name
        let name = parts[0].trim();
        if name.is_empty() {
            return Person::default();
        }

        // 4. Parse the second element as the age
        ...
        // 5. If parsing fails, return the default
        Person::default()
    }
}
